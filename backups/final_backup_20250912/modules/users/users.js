...
class UsersModule {
    ...
    showAddUserModal() {
        ...
        if (modalTitle) {
            modalTitle.textContent = '새 사용자 추가';
            console.log('[Users] 모달 제목 설정됨');
        } else {
            console.error('[Users] modal-title 요소를 찾을 수 없음');
        }
        
        if (userForm) {
            userForm.reset();
            console.log('[Users] 사용자 폼 초기화됨');
        } else {
            console.error('[Users] user-form 요소를 찾을 수 없음');
        }
        ...
        if (userModal) {
            ...
            console.log('[Users] 사용자 모달 표시됨');
            console.log('[Users] 모달 현재 클래스:', userModal.className);
            console.log('[Users] 모달 현재 스타일:', userModal.style.cssText);
        } else {
            console.error('[Users] user-modal 요소를 찾을 수 없음');
        }
    }
        ...
        try {
            ...
            if (user) {
                ...
                const fields = {
                    ...
                    'user-password': '', // 비밀번호는 비움
                    'user-role': user.role,
                    'user-contact': user.contact_info || '',
                    'user-department': user.department || '',
                    'user-position': user.position || '',
                    'user-managed-site': user.managed_site || ''
                };
                ...
                const operatorCheckbox = document.getElementById('user-operator');
                const semiOperatorCheckbox = document.getElementById('user-semi-operator');
                if (operatorCheckbox) operatorCheckbox.checked = user.operator || false;
                if (semiOperatorCheckbox) semiOperatorCheckbox.checked = user.semi_operator || false;
                
                const userModal = document.getElementById('user-modal');
                if (userModal) userModal.classList.remove('hidden');
            }
        } catch (error) {
            console.error('[Users] 사용자 정보 로드 실패:', error);
    ...
    async saveUser() {
        ...
        try {
            const url = this.currentEditUserId ? 
                `/api/admin/users/${this.currentEditUserId}` : 
                '/api/admin/users';
            ...
            if (result.success) {
                console.log('[Users] 사용자 저장 성공:', result);
                alert(this.currentEditUserId ? '사용자가 수정되었습니다.' : '새 사용자가 추가되었습니다.');
                this.closeUserModal();
                console.log('[Users] 사용자 목록 새로고침 시작');
                this.loadUsers();
                console.log('[Users] 사용자 목록 새로고침 완료');
            } else {
                alert(result.message || '저장 중 오류가 발생했습니다.');
            }
        } catch (error) {
            console.error('[Users] 사용자 저장 실패:', error);
...
window.resetPassword = (userId) => window.UsersModule.resetPassword(userId);
window.toggleUserStatus = (userId, newStatus) => window.UsersModule.toggleUserStatus(userId, newStatus);
window.manageSites = (userId) => window.UsersModule.manageSites(userId);
window.searchUsers = () => window.UsersModule.searchUsers();
window.changePage = (direction) => window.UsersModule.changePage(direction);
window.loadUsers = () => window.UsersModule.loadUsers();
window.loadManagedSites = () => {
    // 실제 구현이 필요한 경우 추가
    console.log('[Users] loadManagedSites called');
};

// 사용자-사업장 관리 관련 함수들
window.toggleAllSites = (checkbox) => {
...
window.toggleAllSitesModal = (checkbox) => {
