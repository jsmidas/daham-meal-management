...
class User(Base):
    ...
    is_active = Column(Boolean, default=True)  # 사용자 활성 상태
...
class Supplier(Base):
    ...
    # Relationships (ingredients 관계는 문자열 supplier_name 사용으로 제거)
...
# IngredientUploadHistory: 식자재 업로드 히스토리
class IngredientUploadHistory(Base):
    __tablename__ = "ingredient_upload_history"
    
    id = Column(Integer, primary_key=True, index=True)
    filename = Column(String(255), nullable=False)  # 원본 파일명
    uploaded_by = Column(String(50), nullable=False)  # 업로드한 사용자
    upload_date = Column(DateTime, default=func.now())  # 업로드 일시
    total_rows = Column(Integer, default=0)  # 전체 행 수
    processed_count = Column(Integer, default=0)  # 처리된 행 수
    updated_count = Column(Integer, default=0)  # 업데이트된 행 수
    error_count = Column(Integer, default=0)  # 오류 행 수
    status = Column(String(20), default='completed')  # 처리 상태 (processing, completed, failed)
    error_details = Column(JSON)  # 오류 상세 내역
    created_at = Column(DateTime, default=func.now())
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now())

# Ingredient: 식자재 정보 테이블 (정확한 순서와 필수 필드)
class Ingredient(Base):
    __tablename__ = "ingredients"
    
    id = Column(Integer, primary_key=True, index=True)
    
    # 영문 필드명으로 데이터베이스 컬럼 생성 (한글 주석)
    category = Column(String(100), nullable=False, index=True)  # 분류(대분류)
    sub_category = Column(String(100), nullable=False, index=True)  # 기본식자재(세분류)
    ingredient_code = Column(String(50), unique=True, nullable=False, index=True)  # 고유코드
    ingredient_name = Column(String(255), nullable=False, index=True)  # 식자재명
    posting_status = Column(String(20))  # 게시유무
    origin = Column(String(255))  # 원산지
    specification = Column(String(255))  # 규격
    unit = Column(String(20), nullable=False)  # 단위
    tax_type = Column(String(20))  # 면세
    delivery_days = Column(String(20), nullable=False)  # 선발주일
    price_per_gram = Column(DECIMAL(8, 4))  # g당 단가 (원/g)
    purchase_price = Column(DECIMAL(10, 2), nullable=False)  # 입고가
    selling_price = Column(DECIMAL(10, 2), nullable=False)  # 판매가
    supplier_name = Column(String(255), nullable=False, index=True)  # 거래처명
    notes = Column(Text)  # 비고
    created_date = Column(DateTime, default=func.now())  # 등록일
    
    # 여유 컬럼 3개
    extra_field1 = Column(String(255))  # 여유필드1
    extra_field2 = Column(String(255))  # 여유필드2
    extra_field3 = Column(Text)  # 여유필드3
    
    # 시스템 정보
    is_active = Column(Boolean, default=True)  # 활성 상태
    created_by = Column(String(50))  # 등록자
    upload_batch_id = Column(Integer, ForeignKey('ingredient_upload_history.id'))  # 업로드 배치 ID
    created_at = Column(DateTime, default=func.now())
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now())
    
    # 관계 설정 (supplier 관계는 supplier_name 문자열 사용으로 제거)
    upload_history = relationship("IngredientUploadHistory", backref="ingredients")
