...
async function loadMealPlansForLocation() {
    ...
    // 실제 meal_pricing 테이블에서 사업장별 데이터 로드
    try {
        const response = await fetch(`/api/admin/meal-pricing/by-location/${selectedLocationId}`);
        const data = await response.json();
        
        console.log('식단가 API 응답:', data);
        
        if (data.success && data.mealPlans && data.mealPlans.length > 0) {
            // 실제 데이터베이스에서 가져온 데이터 사용
            mealPlans = data.mealPlans.map(pricing => ({
                id: pricing.id,
                name: pricing.name || '세부식단표', // API에서 제공하는 name 필드 사용 (plan_name 매핑됨)
                plan_name: pricing.name, // 추가로 plan_name도 설정
                meal_time: pricing.meal_time, // API에서 meal_time으로 매핑됨
                selling_price: pricing.selling_price,
                target_material_cost: pricing.target_material_cost || 0,
                cost_ratio: pricing.cost_ratio || 0,
                location_id: selectedLocationId,
                location_name: pricing.location_name,
                customer_name: selectedBusinessName
            }));
            console.log(`${selectedBusinessName} 실제 식단가 데이터 ${mealPlans.length}개 로드됨`);
            console.log('식단가 데이터:', mealPlans);
        } else {
            // 해당 사업장에 데이터가 없으면 새 식단표 템플릿 생성
            mealPlans = [
                {
                    id: Date.now(),
                    name: '새 세부식단표',
                    ...
                    meal_plan_type: '중식',
                    selling_price: 5000,
                    target_material_cost: 2500,
                    cost_ratio: 50.0,
                    location_id: selectedLocationId,
                    location_name: selectedBusinessName,
                    customer_name: selectedBusinessName,
            ...
            console.log(`${selectedBusinessName} 새 식단표 템플릿 1개 생성`);
        }
    } catch (error) {
                    ...
                    return `
                        <tr style="background: ${plan.id % 2 === 0 ? '#fafafa' : '#ffffff'}; border-bottom: 1px solid #e0e0e0;">
                            <td style="border: 1px solid #bbbbbb; padding: 8px; text-align: center; font-weight: 500;">
                                ${plan.plan_name || plan.customer_name || '데이터 없음'}
                            </td>
