...
# 식단가 저장 API
@router.post("/meal-pricing/save")
async def save_meal_plans(request: dict, db: Session = Depends(get_db)):
    """식단가 데이터 저장 (JavaScript 연동용)"""
    try:
        location_id = request.get('location_id')
        meal_plans = request.get('meal_plans', [])
        
        if not location_id or not meal_plans:
            return {"success": False, "error": "location_id와 meal_plans는 필수입니다"}
        
        # 기존 데이터 삭제 (해당 location_id의 모든 식단가)
        delete_query = text("DELETE FROM meal_pricing WHERE location_id = :location_id")
        ...
        for plan in meal_plans:
            # 임시 ID(음수)는 새 데이터로 처리
            if plan.get('id', 0) < 0:
                plan['id'] = None  # 새 레코드는 ID를 None으로 설정
            
            insert_query = text("""
                INSERT INTO meal_pricing (
                    location_id, location_name, meal_plan_type, meal_type, 
                    plan_name, selling_price, material_cost_guideline, cost_ratio,
                    apply_date_start, apply_date_end, is_active, created_at, updated_at
                ) VALUES (
                    :location_id, :location_name, :meal_plan_type, :meal_type,
                    :plan_name, :selling_price, :material_cost_guideline, :cost_ratio,
                    :apply_date_start, :apply_date_end, 1, :created_at, :updated_at
                )
            """)
            
            # meal_time을 meal_plan_type으로도 사용
            meal_plan_type_map = {
                'breakfast': '조식',
                'lunch': '중식', 
                'dinner': '석식',
            ...
            db.execute(insert_query, {
                ...
                "meal_plan_type": meal_plan_type_map.get(plan.get('meal_time', 'lunch'), '중식'),
                "meal_type": plan.get('meal_time', 'lunch'),
                "plan_name": plan.get('plan_name', '기본식단표'),
                "selling_price": plan.get('selling_price', 0),
                "material_cost_guideline": plan.get('target_material_cost', 0),
                "cost_ratio": plan.get('cost_ratio', 0),
                "apply_date_start": plan.get('apply_date_start', '2025-01-01'),
                "apply_date_end": plan.get('apply_date_end', '2025-12-31'),
        ...
        return {
            "success": True,
            "message": f"{insert_count}개의 식단가가 저장되었습니다",
            "count": insert_count
        }
