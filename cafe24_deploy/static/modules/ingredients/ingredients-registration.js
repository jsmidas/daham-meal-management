...
window.IngredientsModule = {
    ...
    async loadIngredients() {
        try {
            // Excel 시스템32.JPG의 실제 데이터 사용
            const testIngredients = [
                {
                    category: '가금류',
                    subcategory: '기타가금류',
                    code: '1000067464',
                    name: '밀가루대만두,주고쇼빵,150G/PAC',
                    origin: '',
                    calculation: '',
                    specification: '종합빵,15CPAC',
                    base_unit: 'Full tax',
                    tax_free: 'D-1',
                    selective_order: '490',
                    purchase_price: '540',
            ...
            this.displayIngredients(testIngredients);
            ...
            if (tbody) {
                tbody.innerHTML = '<tr><td colspan="14">식자재 목록을 불러올 수 없습니다.</td></tr>';
            }
        }
    },
    ...
    async loadIngredientStatistics() {
        try {
            // Excel 데이터 기반 통계
            const ingredients = [
                { category: '가금류', posting_status: '게시' },
                { category: '가금류', posting_status: '게시' },
                { category: '가금류', posting_status: '게시' }
            ];
    ...
    displayIngredients(ingredients) {
        ...
        if (!ingredients || ingredients.length === 0) {
            tbody.innerHTML = '<tr><td colspan="14">등록된 식자재가 없습니다.</td></tr>';
            return;
        }
        
        tbody.innerHTML = ingredients.map((ingredient, index) => `
            <tr>
                <td>${ingredient.category || '-'}</td>
                <td>${ingredient.subcategory || '-'}</td>
                <td>${ingredient.code || '-'}</td>
                <td title="${ingredient.name || '-'}" style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${ingredient.name || '-'}</td>
                <td>${ingredient.origin || '-'}</td>
                <td>${ingredient.calculation || '-'}</td>
                <td title="${ingredient.specification || '-'}" style="max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${ingredient.specification || '-'}</td>
                <td>${ingredient.base_unit || '-'}</td>
                <td>${ingredient.tax_free || '-'}</td>
                <td>${ingredient.selective_order || '-'}</td>
                <td>${ingredient.purchase_price ? '₩' + Number(ingredient.purchase_price).toLocaleString() : '-'}</td>
                <td>${ingredient.price ? '₩' + Number(ingredient.price).toLocaleString() : '-'}</td>
                <td>${ingredient.supplier_name || '-'}</td>
                <td>${ingredient.memo || '-'}</td>
            </tr>
        `).join('');
    },
