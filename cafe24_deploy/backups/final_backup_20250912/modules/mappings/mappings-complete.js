...
window.MappingsModule = {
    ...
    displayMappings(mappings, customers, suppliers) {
        this.buildDynamicMatrix(mappings, customers, suppliers);
    },

    // 동적 매트릭스 생성
    buildDynamicMatrix(mappings, customers, suppliers) {
        const tableHead = document.getElementById('matrix-table-head');
        const tableBody = document.getElementById('matrix-table-body');
        
        if (!tableHead || !tableBody) return;

        // 동적 사업장 목록 (API에서 가져온 데이터 사용)
        const dynamicCustomers = customers && customers.length > 0 ? customers : [];

        // 헤더 생성 (사업장 고정, 협력업체 동적)
        let headerHTML = '<tr><th class="customer-column-header">사업장 \\ 협력업체</th>';
        suppliers.forEach(supplier => {
            headerHTML += `<th class="supplier-header-cell" data-supplier="${supplier.id}">
                ${supplier.name}<br><small>${supplier.supplier_code || ''}</small>
            </th>`;
        });
        headerHTML += '</tr>';
        ...
        dynamicCustomers.forEach(customer => {
            bodyHTML += `<tr><td class="customer-name-cell">${customer.name}</td>`;
            
            suppliers.forEach(supplier => {
                // 해당 고객-공급업체 조합의 매핑 찾기
                const mapping = mappings.find(m => 
                    m.customer_id === customer.id && m.supplier_id === supplier.id
                );
                
                if (mapping && mapping.delivery_code) {
                    // 활성 매핑
                    bodyHTML += `<td class="mapping-cell active" data-customer="${customer.id}" data-supplier="${supplier.id}" data-delivery-code="${mapping.delivery_code}">
                        <div class="delivery-code">${mapping.delivery_code}</div>
                    </td>`;
                } else {
                    // 빈 매핑
                    bodyHTML += `<td class="mapping-cell empty" data-customer="${customer.id}" data-supplier="${supplier.id}" data-delivery-code="">
                        <div class="add-mapping">+</div>
                    </td>`;
                }
            });
        ...
        console.log('동적 매트릭스 생성 완료:', {
            고객수: dynamicCustomers.length,
            공급업체수: suppliers.length,
            매핑수: mappings.length
        });
    },
