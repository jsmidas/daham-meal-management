...
window.IngredientsModule = {
    ...
    async loadIngredients() {
        try {
            ...
            let url = `/api/admin/ingredients-new?page=${page}&limit=200000`;  // 전체 데이터 표시 (최대 20만건)
    ...
    async loadIngredientStatistics() {
        try {
            const response = await fetch(`/api/admin/ingredients-new?page=1&limit=200000`);  // 전체 통계용 (최대 20만건)
            const data = await response.json();
