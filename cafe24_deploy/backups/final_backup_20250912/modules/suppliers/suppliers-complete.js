...
function showAddSupplierModal() {
    ...
    if (supplierModal) {
        ...
        // 모달 폼 강제 스타일 적용
        const modalForm = supplierModal.querySelector('form');
        if (modalForm) {
        ...
        // 폼 그룹들 강제 스타일 적용
        const formGroups = supplierModal.querySelectorAll('.form-group');
        ...
        // 체크박스 라벨 특별 스타일 적용
        const checkboxLabels = supplierModal.querySelectorAll('.checkbox-label');
        ...
        // 체크박스 입력 필드 특별 스타일
        const checkboxes = supplierModal.querySelectorAll('input[type="checkbox"]');
        ...
        // 입력 필드들 강제 스타일 적용 (체크박스 제외)
        const inputs = supplierModal.querySelectorAll('input:not([type="checkbox"]), select, textarea');
        ...
        console.log('[Suppliers] 공급업체 모달 CSS 강제 적용 완료:', supplierModal.style.cssText);
...
async function editSupplier(supplierId) {
    try {
        console.log('[EditSupplier] 협력업체 편집 시작:', supplierId);
        const response = await fetch(`/api/admin/suppliers/${supplierId}/detail`);
        const result = await response.json();
        
        console.log('[EditSupplier] API 응답 상태:', response.status);
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}: ${response.statusText}`);
        }
        console.log('[EditSupplier] API 응답 전체:', result);
        
        const supplier = result.supplier || result.data || result;
        console.log('[EditSupplier] 추출된 supplier 데이터:', supplier);
        
        if (supplier) {
            currentEditSupplierId = supplierId;
            document.getElementById('supplier-modal-title').textContent = '공급업체 정보 수정';
            
            // 폼에 기존 데이터 채우기 - NULL 값 처리 개선
            console.log('[EditSupplier] 데이터 매핑 시작');
            
            // helper function for null/undefined values
            ...
            // 기본 정보
            const supplierIdValue = getValue(supplier.id);
            const supplierName = getValue(supplier.name);
            const businessType = getValue(supplier.business_type);
            const representative = getValue(supplier.representative);
            const businessNumber = getValue(supplier.business_number);
            const businessItem = getValue(supplier.business_item);
            
            console.log('[EditSupplier] 추출된 기본 정보:');
            console.log('- ID:', supplierIdValue);
            console.log('- 이름:', supplierName);
            console.log('- 유형:', businessType);
            console.log('- 대표자:', representative);
            
            document.getElementById('supplier-id').value = supplierIdValue;
            document.getElementById('supplier-name').value = supplierName;
            document.getElementById('supplier-business-type').value = businessType;
            document.getElementById('supplier-representative').value = representative;
            document.getElementById('supplier-business-number').value = businessNumber;
            document.getElementById('supplier-business-item').value = businessItem;
            
            // 연락처 정보 - DB 필드명과 정확히 매핑
            document.getElementById('supplier-phone').value = getValue(supplier.headquarters_phone);
            document.getElementById('supplier-fax').value = getValue(supplier.headquarters_fax);
            document.getElementById('supplier-email').value = getValue(supplier.email);
            document.getElementById('supplier-website').value = getValue(supplier.website);
            document.getElementById('supplier-address').value = getValue(supplier.headquarters_address);
            
            // 추가 정보
            document.getElementById('supplier-parent-code').value = getValue(supplier.parent_code);
            document.getElementById('supplier-company-scale').value = getValue(supplier.company_scale);
            
            // 운영 설정
            document.getElementById('supplier-update-frequency').value = 'weekly'; // 기본값
            document.getElementById('supplier-is-active').checked = supplier.is_active !== false && supplier.is_active !== 0;
            document.getElementById('supplier-notes').value = getValue(supplier.notes);
            
            console.log('[EditSupplier] 폼에 채워진 데이터:');
            console.log('- 이름:', document.getElementById('supplier-name').value);
            console.log('- 유형:', document.getElementById('supplier-business-type').value);
            console.log('- 전화:', document.getElementById('supplier-phone').value);
            console.log('- 주소:', document.getElementById('supplier-address').value);
            
            // 모달 표시 - showAddSupplierModal과 동일한 방식으로 스타일 적용
            const supplierModal = document.getElementById('supplier-modal');
            if (supplierModal) {
                supplierModal.classList.remove('hidden');
                // 강제로 display 스타일 설정
                ...
                console.log('[Suppliers] 편집 모달 CSS 강제 적용 완료:', supplierModal.style.cssText);
            }
        }
    } catch (error) {
        console.error('[EditSupplier] 공급업체 정보 로드 실패:', error);
        console.error('[EditSupplier] 에러 상세:', {
            message: error.message,
            stack: error.stack,
            supplierId: supplierId
        });
        alert('공급업체 정보를 불러올 수 없습니다: ' + error.message);
    }
}
...
function closeSupplierModal() {
    console.log('[CloseModal] 협력업체 모달 닫기');
    const modal = document.getElementById('supplier-modal');
    if (modal) {
        // 강제 스타일 제거하고 hidden 클래스 추가
