...
class MenuRecipeManagement {
    constructor() {
        ...
        // config.js의 구조: CONFIG.API.BASE_URL
        this.CONFIG = {
            API_BASE_URL: window.CONFIG?.API?.BASE_URL || 'http://127.0.0.1:8010'
        };
    ...
    initGrid() {
        ...
        this.hot = new Handsontable(container, {
            ...
            colWidths: [90, 280, 35, 160, 50, 60, 75, 75, 85, 100, 70],
            rowHeaders: true,
            height: 350,
    ...
    /**
     * 식자재 데이터 로드 - 초기에는 샘플 데이터 표시
     */
    async loadIngredients() {
        const tbody = document.getElementById('searchResultsBody');
        if (!tbody) return;

        tbody.innerHTML = '<tr><td colspan="10" style="text-align: center; padding: 20px;">샘플 데이터 로드 중...</td></tr>';

        try {
            // 초기 로드시 샘플 데이터 표시 (50개 정도)
            const url = `${this.CONFIG.API_BASE_URL}/api/admin/ingredients-new?page=1&per_page=50`;
            const response = await fetch(url);
            const data = await response.json();

            if (data.success && data.ingredients) {
                let filtered = data.ingredients.filter(item => item.selling_price && item.selling_price > 0);
                ...
                tbody.innerHTML = `
                    <tr style="background: #f8f9fa;">
                        <td colspan="10" style="text-align: center; padding: 10px; font-size: 12px; color: #6c757d;">
                            <strong>샘플 데이터 ${filtered.length}개</strong> (더 많은 결과를 보려면 검색해주세요)
                        </td>
                    </tr>
                ` + filtered.map((item, index) => `
                    <tr onclick="window.selectIngredientByIndex(${index})" style="cursor: pointer;">
                ...
                tbody.innerHTML = '<tr><td colspan="10" style="text-align: center; padding: 20px; color: red;">데이터를 불러올 수 없습니다.</td></tr>';
            }
        } catch (error) {
            console.error('샘플 데이터 로드 실패:', error);
            tbody.innerHTML = '<tr><td colspan="10" style="text-align: center; padding: 20px; color: red;">데이터 로드 중 오류가 발생했습니다.</td></tr>';
        }
    }
    ...
    async searchIngredients() {
        ...
        if (!searchTerm && !supplierFilter) {
            tbody.innerHTML = '<tr><td colspan="10" style="text-align: center; padding: 30px; color: #6c757d;">식자재명 또는 코드를 검색해주세요.<br><small style="color: #adb5bd;">84,000여개의 식자재를 검색할 수 있습니다.</small></td></tr>';
            return;
        }
        ...
        try {
            // 서버에 검색 요청 - 검색할 때는 전체 84,000개에서 검색
            let url = `${this.CONFIG.API_BASE_URL}/api/admin/ingredients-new?page=1&per_page=50000`;
            if (searchTerm) {
    ...
    async loadMenus() {
        try {
            ...
            if (!response.ok) {
                ...
                response = await fetch(`${this.CONFIG.API_BASE_URL}/api/admin/menu-recipes`, {
                    headers: headers
                });
                useAdminApi = true;
            }
        ...
        menuList.innerHTML = sortedMenus.map((menu, index) => {
            ...
            return `
                <div class="menu-item" onclick="selectMenu(${menu.id}, this)" style="display: flex; align-items: center; gap: 10px; padding: 10px; cursor: pointer; border-bottom: 1px solid #eee; ${bgColor ? `background-color: ${bgColor};` : ''}">
                    <div style="width: 24px; height: 24px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; background: #e9ecef; color: #495057; font-size: 11px; font-weight: 600; border-radius: 50%; margin-right: 6px;">
                        ${menuNumber}
                    </div>
                    ...
                    <div style="flex: 1;">
                        <h4 style="margin: 0; font-size: 14px;">
                            ${indicator}
                            ${menu.name || menu.menu_name || menu.recipe_name || '이름 없음'}
                        </h4>
    ...
    searchMenus() {
        ...
        const filtered = this.menus.filter(menu => {
            const menuName = menu.name || menu.menu_name || menu.recipe_name || '';
            return menuName.toLowerCase().includes(searchTerm);
        });
        ...
        menuList.innerHTML = filtered.map(menu => `
            <div class="menu-item" onclick="selectMenu(${menu.id}, this)" style="display: flex; align-items: center; gap: 10px; padding: 10px; cursor: pointer; border-bottom: 1px solid #eee;">
                ...
                <div style="flex: 1;">
                    <h4 style="margin: 0; font-size: 14px;">${menu.name || menu.menu_name || menu.recipe_name || '이름 없음'}</h4>
                    <small style="color: #666;">${menu.category || '미분류'}</small>
                </div>
            </div>
        `).join('');
    ...
    async selectMenu(menuId, element) {
        ...
        if (menu) {
            ...
            const menuName = menu.name || menu.menu_name || menu.recipe_name || '';
            if (menuTitleElement) menuTitleElement.textContent = menuName;
            if (menuNameElement) menuNameElement.value = menuName;
